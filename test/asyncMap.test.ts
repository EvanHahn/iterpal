...
Deno.test(
  "returns a new iterator with values mapped, using a synchronous fn",
  async () => {
...
Deno.test(
  "returns a new iterator with values mapped, using an asynchronous fn",
  async () => {
    const fn = spy((n: number) => Promise.resolve(n * n));
    const several = {
      async *[Symbol.asyncIterator]() {
        yield 1;
        yield 2;
